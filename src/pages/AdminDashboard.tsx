...
import { Shield, Trash2, Eye, Bell, BellOff, AlertTriangle, CheckCircle, XCircle, BarChart3, Calendar, TrendingUp, Video, MousePointerClick, Search } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const fetchEvents = async () => {
    const { data } = await supabase.from("events").select("*, categories(name)").order("created_at", { ascending: false });
    setEvents(data || []);
  };

  const fetchPendingEvents = async () => {
    const { data } = await supabase.from("events").select("*, categories(name)").eq("status", "pending").order("created_at", { ascending: false });
    setPendingEvents(data || []);
  };

  const fetchNotifications = async () => {
    const { data } = await supabase.from("admin_notifications").select("*, events(title, organizer_name)").eq("is_read", false).order("created_at", { ascending: false });
  ...
  const approveEvent = async (eventId: string) => {
    const { error } = await supabase.from("events").update({ status: "approved", is_published: true, last_reviewed_at: new Date().toISOString(), updated_by_admin: true }).eq("id", eventId);
    if (!error) { toast({ title: "Événement approuvé et publié !" }); fetchAll(); }
  };

  const rejectEvent = async (eventId: string) => {
    const { error } = await supabase.from("events").update({ status: "rejected", is_published: false, last_reviewed_at: new Date().toISOString(), updated_by_admin: true }).eq("id", eventId);
    if (!error) { toast({ title: "Événement rejeté" }); fetchAll(); }
  };

  const togglePublish = async (eventId: string, currentStatus: boolean) => {
    const { error } = await supabase.from("events").update({ is_published: !currentStatus, updated_by_admin: true }).eq("id", eventId);
  ...
  const toggleLive = async (eventId: string, currentLive: boolean) => {
    const { error } = await supabase.from("events").update({ is_live: !currentLive, updated_by_admin: true }).eq("id", eventId);
    if (!error) { toast({ title: currentLive ? "Live désactivé" : "Live activé" }); fetchEvents(); }
  };
  ...
  const filteredAllEvents = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(e => e.title?.toLowerCase().includes(q) || e.city?.toLowerCase().includes(q) || e.organizer_name?.toLowerCase().includes(q));
    }
    return list;
  ...
  const FilterBar = () => (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center mb-4">
      <div className="flex items-center gap-2 flex-1 min-w-0 px-3 py-2 rounded-lg bg-muted">
        ...
        <Input placeholder="Rechercher..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setAllEventsPage(1); setPendingPage(1); }}
      ...
      <div className="pb-16 pt-20">
        <div className="container mx-auto px-4">
          ...
          <Tabs defaultValue="pending" className="space-y-4 sm:space-y-6">
            <TabsList className="grid h-auto w-full grid-cols-3 gap-1 bg-muted p-1 md:w-auto md:grid-cols-7">
            ...
            <TabsContent value="pending">
              <Card>
                <CardHeader>
                  <CardTitle className="font-display text-base sm:text-lg">Événements en attente ({filteredPendingEvents.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  <FilterBar />
            ...
            <TabsContent value="all">
              <Card>
                <CardHeader>
                  <CardTitle className="font-display text-base sm:text-lg">Tous les événements ({filteredAllEvents.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  <FilterBar />
                  <div className="space-y-2 sm:space-y-3">
