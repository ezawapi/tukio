...
const PromotionalBanner = () => {
  ...
  if (!banner.button_url) return <div className="container mx-auto w-full px-4 md:w-[80%] md:px-0 max-w-6xl">{content}</div>;

  if (isExternal) {
    return (
      <div className="container mx-auto w-full px-4 md:w-[80%] md:px-0 max-w-6xl">
        <a href={banner.button_url} target="_blank" rel="noopener noreferrer" className="block">{content}</a>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto w-full px-4 md:w-[80%] md:px-0 max-w-6xl">
      <Link to={banner.button_url} className="block">{content}</Link>
    </div>
  );
};
