...
const NearbyEvents = () => {
  ...
  useEffect(() => {
    ...
    // Use watchPosition for better accuracy — takes the first good reading
    let resolved = false;
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Stop watching after a good fix
        ...
        <div className="mb-6 flex items-end justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 font-body text-sm text-muted-foreground">Événements les plus proches de votre position</p>
            </div>
          </div>
                ...
                <Link to={`/events/${event.id}`} className="group flex gap-3 rounded-xl bg-background p-2.5 shadow-card transition-shadow hover:shadow-warm sm:gap-4 sm:p-3">
                  ...
                  <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      {countdown && (
                        <Badge className="border-0 bg-primary/10 text-[9px] font-bold text-primary sm:text-[10px]">
                          {countdown}
                        </Badge>
                      )}
                    </div>
